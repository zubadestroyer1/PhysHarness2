...
import { refreshWorkspace, replaceRecord } from './workspace'
...
function Activity({ events, pending }: { events: WorkspaceData['events']; pending: boolean }) {
  return <><PageHeader eyebrow="Append-only log" title="Activity" description="Activity starts with up to 100 recent visible events, then retains events observed in this connection. Earlier history is not loaded." /><p className="muted">{pending ? 'Catching up with additional event pages; displayed records may be behind.' : 'Events include correlated operation IDs and stored payloads.'}</p>{events.length ? <ol className="timeline">{[...events].sort((a, b) => b.sequence - a.sequence).map(item => <li key={item.sequence}><span className="timeline-marker" /><div><header><strong>{item.kind}</strong><time>{formatDate(item.created_at)}</time></header><p>Aggregate <code>{item.aggregate_id}</code></p><details><summary>Event #{item.sequence} · operation {item.operation_id}</summary><pre>{JSON.stringify(item.payload, null, 2)}</pre></details></div></li>)}</ol> : <EmptyState title="No activity events" body="The server returned an empty event collection. New canonical operations will appear here." />}</>
}
