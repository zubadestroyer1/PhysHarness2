...
A forward event page is committed with its cursor only after the associated canonical refresh succeeds. Empty visible pages still advance using the server cursor. Additional pages wait for subsequent cycles, and the sidebar and Activity page show when catch-up is pending. Activity starts with up to 100 recent visible events, then keeps only the 1,000 newest observed events in sequence order. The UI labels this display limit; older history is omitted. Trimming this display window does not change the independent server cursor, mark unread events as processed, or truncate canonical record inventories. Empty event pages reuse the retained window. Problems, experiments, claims, branches and experiment-owned artifacts are displayed within the chosen campaign; unscoped artifacts remain visible.

Selected record details and transition revisions come from refreshed canonical records. The selected experiment ledger refreshes on selection and every poll. The Overview metric is a project-wide total of recorded sessions, including finished sessions; active worker counts remain separate in the selected experiment ledger. Each unique canonical runtime session save emits `session.saved` with its canonical record ID and experiment/task scope in the same fenced, idempotent transaction. Incremental refresh reads that session on creation and subsequent status updates; replayed saves emit no duplicate event. Loaded immutable artifact content remains bound to its artifact ID while metadata refreshes. Session, selection and request guards prevent late ledger, artifact, export, mutation and collection completions from replacing a newer view or repopulating a disconnected session. Closing or reopening an export invalidates the earlier export request.
