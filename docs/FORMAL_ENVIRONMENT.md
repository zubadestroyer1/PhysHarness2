...
| Mathlib | `db584cd6d46c92f209a44c0f1c829460d327499d` | Physlib's exact resolved 4.33.0 dependency |
| Physlib | `405848179db6375f814021e804a37dc0dda66d91` | Declares Lean 4.33.0; selected source build target provided |
| QuantumInfo | Same Physlib commit | Included in the same source tree and Lake project |
...
Mathlib root, so the selected closure is approximately 8,700 source modules. It is
...
before changing dependency locations to `/opt/sources/<package>`. Upstream Lean settings
are preserved. No Mathlib olean cache is downloaded by this build recipe. The lock records source-resolution status; later build and engineering results live in
