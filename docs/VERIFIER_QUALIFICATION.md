...
- The current profile gives Comparator 600 seconds and caps its output at 256,000 bytes. Earlier Wave 1 evidence used the historical 2 GiB profile and remains evidence only for those recorded hashes. The [final 8 GiB controls](../work/wave01/current-evidence-audit.md) cover 24/24 expected core/library outcomes across both modes, 16/16 fixed observations, and 224 passed scoped host regressions with one conditional PostgreSQL skip. All 10 automatic checks are mechanically `satisfied`. The separate physics benchmark recorded 60/60 expected reference/control outcomes in each mode; the stored full-suite assessment matched independent audit. Human scientific and deployment gates remain pending.
- The recorded 8 GiB scope predates the combined PR #21 source tree. Its mechanical assessment is historical for those exact hashes; current-source qualification needs fresh scoped evidence and review.
