...
The existing system has useful foundations: stable workflow identities, a PostgreSQL transactional outbox with `SKIP LOCKED`, leases with fencing, resource reservations, immutable artifacts, model-directed delegation, and independent proof receipts. Those are worth keeping. The [implementation evidence ledger](../docs/IMPLEMENTATION_STATUS.md) explicitly leaves live providers, the acceptance boundary, 128-worker operation, and 1,000-worker operation unqualified.

| Observed implementation | Scaling implication |
|---|---|
| [One worker process](../src/physharness/worker.py#L164) registers experiment, research, and verification activities on one task queue; it also pumps the outbox. | Split these into independently deployable pools and queues. A 16-thread executor applies to synchronous activities; it is **not** evidence of either a 16-agent fleet limit or sufficient capacity for 128 agents. |
| [Research tasks](../src/physharness/orchestration/workflows.py#L40) run as activities with one attempt, a 24-hour timeout, and heartbeats. Preflight waiting is retried every five seconds. | Deliberately conservative side-effect handling is appropriate. A pod restart cannot safely turn into an automatic new research attempt. Replace repeated eligibility polling with admission/wakeup mechanisms as queue pressure grows. |
| [Prior model sessions block redispatch](../src/physharness/orchestration/research_worker.py#L295) with `RECOVERY_RECONCILIATION_REQUIRED`. | Safe continuation after worker loss is unfinished. Buying a scheduler with automatic restarts does not finish it. |
| The distributed executor [accepts only the Responses runtime](../src/physharness/orchestration/research_worker.py#L312); Codex, Claude, and OpenHands adapters exist separately. | Heterogeneous model-runtime scaling requires controller integration and capability qualification, not merely SDK installation. |
| [WorkspaceBroker requires `provider == "e2b"`](../src/physharness/orchestration/workspaces.py#L128), imports E2B's archive type, and requires `provider_vm` isolation. | Alternative providers are feasible, but **not drop-in configuration changes**. Separate the lifecycle contract, archive format, isolation description, and provider factory. Do not relabel gVisor as a VM to pass a capability check. |
| [Routine workspace cleanup](../src/physharness/orchestration/workspace_tools.py#L85) supplies `actual_cost_usd=None`. | Confirmed destruction can release capacity while financial reservations remain unresolved. A busy fleet needs automated billing reconciliation or conservative, explicitly recorded settlement policies. Otherwise reserved budget accumulates even after successful cleanup. |
| [Portable archives](../src/physharness/execution/e2b.py#L26) cap the serialized archive at 65,536 bytes, individual files at 32,768 bytes, and file count at 64. Native child allocation is rejected by the canonical journal. | Full Lean/Mathlib trees need a versioned streaming artifact design. Provider-native fork is not usable at fleet level until child reservations, lineage, adoption, and cleanup are integrated. |
| [Async research execution](../src/physharness/orchestration/research_worker.py#L290) calls synchronous service/database methods and [synchronous S3 I/O](../src/physharness/artifacts.py#L107). | Network or database latency can block the event loop and delay renewals. Instrument event-loop lag, then move blocking work into appropriately bounded execution paths. Adding replicas does not remove this per-process risk. |
| [Resource reservations](../src/physharness/service.py#L695) lock experiment and budget rows; [lease renewal](../src/physharness/collaboration.py#L143) also locks the experiment via `_active`. | Test one large experiment as well as many small ones. The same-experiment hot row may constrain concurrency before the infrastructure scheduler does. Preserve atomic budget guarantees when optimizing. |
| Session and workspace discovery in the executor enumerates experiment records and filters by task/slot in Python. [Executor](../src/physharness/orchestration/research_worker.py#L295). | Add indexed task/session and slot/workspace queries when measurements confirm the cost. Large campaigns should not repeatedly hydrate unrelated records. |
| [AWS Terraform](../infra/terraform/aws/services.tf) defines fixed API/worker desired counts. [Self-hosting](../infra/selfhost/README.md) remains a qualification outline. | ECS autoscaling, worker draining, Kubernetes manifests, sandbox controllers, and a self-hosted VM allocator are additional work. |
| [The verifier](../src/physharness/verification/container_driver.py#L61) requires a particular Linux boundary including Landlock, seccomp, and syscall probes. | A different VM/container runtime requires requalification. A successful research-shell sandbox test does not qualify independent proof acceptance. |
...
| **OpenHands SDK** | Remote code-agent execution with workspace/conversation state. [SDK](https://www.openhands.dev/product/sdk). | Qualify the existing adapter for Lean/code-heavy work. Complete distributed integration and portable branch-memory handoff; native continuation remains tied to the same qualified VM/runtime, not a replacement VM. [Adapter contract](../docs/OPENHANDS_RUNTIME.md). Adding another framework above it should have a measured reason. |
